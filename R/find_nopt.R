...
find_nopt <- function(sigma0, reporting_prob0, x0 = 1, death_rate, ptree, ptree_lag = 0, sample_prevalence, sigma_mean = 0.1, pobs_prior = "uniform", pobs_min = 0, pobs_max = 1, pobs_alpha = 1, pobs_beta = 1, x0_prior = "uniform", x0_min=1, x0_max=Inf, x0_mean=10, x0_var=100, ess_threshold_prop = 0.5, resampling_scheme = "systematic", print = F) {
  ...
  f_hat_old <- sir(n_particles = n_particles, sigma = sigma_old, x0 = x0_old, death_rate = death_rate, sample_prevalence = sample_prevalence, reporting_prob = p_obs_old, genetic_data = genetic_data, ess_threshold = ess_threshold, resampling_scheme = resampling_scheme, backward_sim = F)$int_llik
  ...
  for (i in 1:500) {
    ...
    f_hat_new <- sir(n_particles = n_particles, sigma = sigma_new, x0 = x0_new, death_rate = death_rate, sample_prevalence = sample_prevalence, reporting_prob = p_obs_new, genetic_data = genetic_data, ess_threshold = ess_threshold, resampling_scheme = resampling_scheme, backward_sim = F)$int_llik
  ...
  for (i in 501:1000) {
    ...
    if (x0_prior == "uniform") {
      ...
      x0_prior_new <- dtnbinom(x = x0_new, size = n_nb, mu = x0_mean, a = x0_min, b = x0_max, log = T)
    } else {
      stop("time 0 prevalence prior should be 'uniform' or 'nbinom'")
    }
    prior_new <- sigma_prior_new + pobs_prior_new + x0_prior_new
  ...
  for (r in 1:R) {
    ...
    nopt_llik[r] <- sir(n_particles = Ns, x0 = max(1, round(x_mean,0)), sigma = sig_mean, death_rate = death_rate, sample_prevalence = sample_prevalence, reporting_prob = p_obs_mean, genetic_data = genetic_data, ess_threshold = Ns*ess_threshold_prop, resampling_scheme = resampling_scheme, backward_sim = F)$int_llik
  }
  #remove -Infs
