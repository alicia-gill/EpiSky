...
#'
#' @examples
#' rtskellam_sir(n = 100, old_x = x_resample, birth_rate = b_sample, death_rate = 0.1)
rtskellam_sir <- function(n, old_x, birth_rate, death_rate, trunc = -old_x) {
  N <- length(old_x)
  B <- length(birth_rate)
  D <- length(death_rate)
  if (N != n) {
  ...
  if (m > 0) {
    for (i in 1:m) {
      ...
      w <- dskellam_sir(new_x = x, rep(old_x[j], 100), birth_rate[j], death_rate, log=T)
