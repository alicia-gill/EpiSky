...
sir <- function(n_particles, ess_threshold = n_particles/2, x0 = 1, death_rate, sigma, reporting_prob, sample_prevalence, genetic_data, resampling_scheme = "systematic", backward_sim = TRUE) {
  ...
  #if reporting_prob is 0, we want to sample entirely from the prior
  #if reporting_prob is 1, we want to sample entirely from the data
  #from reporting_prob>=0.5, only sample from the data
  # q <- min(0.5, reporting_prob)
  ...
  for (i in 1:N) {
    ...
    #sample x
    trunc <- sample_prevalence[i+1,2] - x_resample
    if (sample_prevalence[i+1,2] == 0) {
      ...
      if (sum_index == 0) {
        ...
        x_sample <- sample_prevalence[i+1,2] + rnbinom(n = n_particles, size = sample_prevalence[i + 1, 2], p = reporting_prob)
      } else {
        ...
        x_sample[index==1] <- sample_prevalence[i+1,2] + rnbinom(n = sum_index, size = sample_prevalence[i + 1, 2], p = reporting_prob)
      }
      ...
      epi_prior <- dskellam_sir(new_x = x_sample, old_x = x_resample, birth_rate = b_sample, death_rate = death_rate, log = T)
      epi_data <- dnbinom(x = x_sample - sample_prevalence[i + 1, 2], size = sample_prevalence[i + 1, 2], prob = reporting_prob, log = T)
#      epi_proposal <- apply(cbind(log1q + epi_prior, logq + epi_data), 1, matrixStats::logSumExp)
    ...
    if (i < P) {
      ...
      noisy_llik <- dbinom(x = sample_prevalence[i + 1, 2], size = x_sample, prob = reporting_prob, log = T)
    }
  ...
  if (backward_sim == TRUE) {
    ...
    for (t in (N-1):1) {
      ...
      skel <- dskellam_sir(new_x = y, old_x = prevalence[t+1,], birth_rate = x, death_rate = death_rate)
