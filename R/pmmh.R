...
#'
#' Implements an particle marginal Metropolis--Hastings algorithm with adaptive resampling when the epidemic with birth rate varying over time has been partially observed with known proportion. MH adaptation of shape and scale of rho and sigma. Unknown optimum number of particles.
#'
...
#' @param reporting_prob0 initial value of the proportion of cases observed.
...
#' @param x0_prior "uniform" or "nbinom"; prior distribution at time 0 prevalence.
#' @param x0_min minimum for uniform time 0 prevalence prior.
#' @param x0_max maximum for uniform time 0 prevalence prior.
#' @param x0_mean mean for negative binomial time 0 prevalence prior.
#' @param x0_var variance for negative binomial time 0 prevalence prior.
...
#'
#' @return list containing: birth rate, prevalence, time 0 prevalence, linear gaussian variance, reporting probability, acceptance rate, run time in seconds, smc log-likelihood, scale, whether the move was accepted, mu and sigma for the adaptation, and the number of particles used to run the chain.
#' @export
#'
#' @examples
...
pmmh <- function(iter, max_time = Inf, target_acceptance = 0.1, sigma0, reporting_prob0, x0 = 1, death_rate, ptree, ptree_lag = 0, genetic_data = NULL, sample_prevalence, sigma_mean = 0.1, pobs_prior = "uniform", pobs_min = 0, pobs_max = 1, pobs_alpha = 1, pobs_beta = 1, x0_prior = "uniform", x0_min=1, x0_max=Inf, x0_mean=10, x0_var=100, n_particles = NULL, ess_threshold = n_particles/2, min_n_particles = 1000, max_n_particles = 10000, resampling_scheme = "systematic", backward_sim = TRUE, print = F) {
  ...
  sum_noisy <- sum(sample_prevalence[-1,2])
  ...
  if (x0_prior == "uniform") {
    ...
    stop("time 0 prevalence prior should be 'uniform' or 'nbinom'")
  }
  prior_old <- sigma_prior_old + pobs_prior_old + x0_prior_old
  sir <- sir(n_particles = n_particles, sigma = sigma_old, x0 = x0_old, death_rate = death_rate, sample_prevalence = sample_prevalence, reporting_prob = p_obs_old, genetic_data = genetic_data, ess_threshold = ess_threshold, resampling_scheme = resampling_scheme, backward_sim = backward_sim)
  ...
  while (i <= iter & run_time <= max_time) {
    ...
    sir <- sir(n_particles = n_particles, sigma = sigma_new, x0 = x0_new, death_rate = death_rate, sample_prevalence = sample_prevalence, reporting_prob = p_obs_new, genetic_data = genetic_data, ess_threshold = ess_threshold, resampling_scheme = resampling_scheme, backward_sim = backward_sim)
  ...
  output <- list("birth_rate" = b_matrix, "prevalence" = p_matrix, "sigma" = sigma, "x0" = x0_output, "reporting_prob" = p_obs, "acceptance_rate" = n_accepted/(i-1), "run_time" = as.numeric(Sys.time()) - sys_time, "smc_llik" = smc_llik, "s"=scale, "accept"=acceptance, "mu"=mu, "Sigma"=Sigma, "n_particles" = n_particles)
  return(output)
}
